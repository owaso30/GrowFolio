...
{{
  ...
  "image_prompts": [
    {{"source": "brand", "brand_key": "github_copilot", "alt": "日本語alt（サービス名＋記事テーマ）"}},
    {{"source": "brand", "brand_key": "github_copilot", "placeholder": "[IMAGE:main]", "alt": "日本語alt"}}
  ]
}}

image_prompts:
- 1件目は必須（アイキャッチ）。2件目以降は最大{body_images}件まで（不要なら1件のみ）
- **AI感を出さない**: 記事の主題サービスがある場合は `"source": "brand"` + `brand_key` を優先（公式ロゴ合成）。Flux は補助のみ
- brand_key 例: bitradex / github_copilot / cursor / vscode / claude / microsoft_365
- Flux を使う場合も prompt は「編集風・実写風・ミニマル」。3D/ネオン/未来的イラスト/架空UI/架空ロゴは禁止
- 実残高スクショ風・管理画面の伪造は禁止
