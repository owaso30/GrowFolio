...
add_action('init', function () {
    register_post_meta('post', '_ssp_meta_title', [
        'show_in_rest' => true,
        'single'       => true,
        'type'         => 'string',
    ]);
    register_post_meta('post', '_ssp_meta_description', [
        'show_in_rest' => true,
        'single'       => true,
        'type'         => 'string',
    ]);
...
| 送る項目        | WordPress 内部キー          |
| ----------- | ----------------------- |
| SEO タイトル    | `_ssp_meta_title`       |
| メタディスクリプション | `_ssp_meta_description` |
...
`meta` オブジェクト内に `_ssp_meta_title` / `_ssp_meta_description` が見えれば OK です。
