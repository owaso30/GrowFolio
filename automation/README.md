...
```
週次: keyword-research.yml  →  KWキュー更新（重複は rewrite_candidate）
週次: fetch-analytics.yml   →  GSC + GA4 + opportunities をキューへ
毎月: sync-posts.yml        →  公開済み記事マスタ同期
火金: publish-scheduled.yml →  クォータ内の記事1本を生成・公開
```
...
| `[keyword-research.yml](../.github/workflows/keyword-research.yml)`   | 毎週日曜 03:00 JST    | トレンド＋サジェスト → KWキュー更新（同意図は除外） |
| `[fetch-analytics.yml](../.github/workflows/fetch-analytics.yml)`     | 毎週月曜 03:00 JST    | GSC/GA4取得＋ opportunities をキュー投入 |
| `[publish-scheduled.yml](../.github/workflows/publish-scheduled.yml)` | **火・金 09:00 JST** | クォータを満たす記事1本を生成・公開                   |
...
python -m pipeline fetch-analytics --dry-run
python -m pipeline fetch-analytics            # 取得後に GSC opportunities をキューへ
python -m pipeline seed-from-analytics       # latest.json だけからキュー投入する場合
